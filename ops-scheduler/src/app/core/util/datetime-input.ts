...
/**
 * Formats a stored UTC ISO timestamp for editing as a local ISO 8601 string with millisecond
 * precision and no offset suffix, e.g. "2026-07-02T14:30:00.000".
 */
export function formatDateTimeInput(isoTime: string): string {
  const local = DateTime.fromISO(isoTime).toLocal();
  return local.isValid ? (local.toISO({ includeOffset: false }) ?? '') : '';
}

/**
 * Parses a local ISO 8601 date-time typed by the user (e.g. "2026-07-02T14:30:00.000") into a UTC
...
export function parseDateTimeInput(value: string): string {
  const dt = DateTime.fromISO(value.trim(), { zone: 'local' });
  return dt.isValid ? (dt.toUTC().toISO() ?? INVALID_DATE_TIME) : INVALID_DATE_TIME;
}

/** Current local time as an ISO 8601 string, truncated to the minute, for pre-filling new-record forms. */
export function nowDateTimeInput(): string {
  return DateTime.local().set({ second: 0, millisecond: 0 }).toISO({ includeOffset: false }) ?? '';
}
